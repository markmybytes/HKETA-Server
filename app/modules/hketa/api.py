...
async def kmb_eta(route: str,
    ...
    """
    fetches KMB/LWB buses ETA (by route) from `ETA Data` API
    ...
    Args:
        ...
        session (aiohttp.ClientSession, optional): client session for HTTP connection
...
async def mtr_bus_eta(route: str,
                      lang: Literal["zh", "en"],
                      session: aiohttp.ClientSession = None) -> dict:
    """fetches MTR buses ETA (by route) from `Real-time MTR Bus and Feeder Bus Schedule` API
...
async def mtr_lrt_eta(stop: int, session: aiohttp.ClientSession = None) -> dict:
    """fetches MTR LRTs ETA (by stop) from `Real-time Light Rail train information` API
...
async def mtr_train_eta(route: str,
                        ...
                        lang: Literal["tc", "en"],
                        session: aiohttp.ClientSession = None) -> dict:
    """fetches MTR trains ETA (by route + stop) from `Real-time MTR train information` API
...
async def bravobus_eta(company: Literal["ctb", "nwfb"],
                       ...
                       session: aiohttp.ClientSession = None) -> dict:
    """
    fetches CityBus/NWFB buses ETA (by route) from `Estimated Time of Arrival (ETA) data` API
...
async def mtr_bus_stop_list(session: aiohttp.ClientSession = None) -> list:
    """fetches MTR buses stop list from `MTR Bus & Feeder Bus Stops` API
...
async def mtr_bus_route_list(session: aiohttp.ClientSession = None) -> list: 
    """fetches MTR buses available route list from `MTR Bus & Feeder Bus Routes` API
...
async def mtr_lrt_route_stop_list(session: aiohttp.ClientSession = None) -> list:
    """fetches MTR light rail details (availavle routes & respective stops) from `Light Rail Routes & Stops` API
...
async def mtr_train_route_stop_list(session: aiohttp.ClientSession = None) -> list:
    """fetches MTR trains (availavle routes & respective stops) from `MTR Lines (except Light Rail) & Stations` API
...
async def kmb_route_list(session: aiohttp.ClientSession  = None) -> dict:
    """fetches KMB available route list from `Route List Data` API
...
async def kmb_route_stop_list(
        ...
        services_type: int,
        session: aiohttp.ClientSession = None) -> dict:
    """fetches KMB stop list (by route) from `Route-Stop Data` API
...
async def kmb_stop_details(stop_id: str,
                           session: aiohttp.ClientSession = None) -> dict:
    """fetches KMB stop information from `Stop Data` API
...
async def bravobus_route_list(company: Literal["ctb", "nwfb"],
                              session: aiohttp.ClientSession = None) -> dict:
    """fetches CityBus/NWFB available route list by route from `Route data` API
...
async def bravobus_route_stop_list(
        ...
        dir: Literal["inbound", "outbound"],
        session: aiohttp.ClientSession = None) -> dict:
    """fetches CityBys/NWFB stop list (by route) from `Bus Stop List of specific Route data` API
...
async def bravobus_stop_details(stop_id: str,
                                session: aiohttp.ClientSession = None) -> dict:
    """fetches CityBus/NWFB stop information from `Stop Data` API
